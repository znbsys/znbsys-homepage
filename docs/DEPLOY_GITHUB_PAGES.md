...
![进入 Settings → Pages](images/pages-open-settings.svg)

### 步骤 2：将构建源改为 GitHub Actions

在 **Build and deployment → Source** 下拉框中选择 **GitHub Actions**（选择后自动保存，无需额外按钮）。

![选择 Source 为 GitHub Actions](images/pages-source-select.svg)
...
![免费账号需 Public 仓库](images/repo-public-required.svg)
...
![手动 Run workflow 与状态说明](images/pages-run-workflow.svg)
...
- **Source 必须选 GitHub Actions**：本项目由 `actions/deploy-pages@v4` 发布，而不是从 `gh-pages` 分支或 `` 目录拉取。选成 “Deploy from a branch” 会导致线上内容为空或永远不更新。
...
| 变量 | 规则 | 示例（仓库 `web-ui-desing`，owner `znb`） |
| --- | --- | --- |
| `NEXT_PUBLIC_SITE_URL` | `https://<owner>.github.io`（**只放协议+主机，不含路径**） | `https://znb.github.io` |
| `NEXT_PUBLIC_BASE_PATH` | 仓库名以 `.github.io` 结尾（用户站）→ 空；否则 → `/<repo>` | `/web-ui-desing` |
...
| 仓库类型 | 站点地址 | basePath |
| --- | --- | --- |
| 项目站（如 `znb/web-ui-desing`） | `https://znb.github.io/web-ui-desing/` | `/web-ui-desing` |
...
STATIC_EXPORT=1 NEXT_PUBLIC_BASE_PATH=/web-ui-desing npm run build
...
- [ ] 页面源码中 canonical 形如 `https://<owner>.github.io/web-ui-desing/zh-CN/`
- [ ] favicon、JS/CSS 请求路径带 `/web-ui-desing` 前缀
